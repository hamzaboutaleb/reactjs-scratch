...
export function arraysDiffSequence(
  ...
  for (let i = 0; i < newArray.length; i++) {
    ...
    const moveOp = array.moveItem(newItem, i);
    sequences.push(moveOp);
  }
...
class ArrayWithOriginalIndices {
  ...
  isAddition(item, startIdx) {
    return this.findIndexFrom(item, startIdx) === -1;
  }
  ...
  findIndexFrom(item, startIdx) {
    for (let i = startIdx; i < this.length; i++) {
      let current = this.#array[i];
      if (this.#equalfn(item, current)) return i;
    }
    return -1;
  }
  ...
  moveItem(item, toIndex) {
    ...
    const [_pos] = this.#indices.splice(fromIndex, 1);

    this.#array.splice(toIndex, 0, _item);
    this.#indices.splice(toIndex, 0, _pos);
